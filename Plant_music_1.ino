...
Through sampling pulse widths and identifying fluctuations, MIDI note and
control messages are generated.  Features include Threshold, Scaling, Control
Number, and Control Voltage using PWM through an RC Low Pass filter.
MIDIsprout.com
-------------*/
...
LEDFader leds[LED_NUM] = {
    ...
    LEDFader(3), LEDFader(5), LEDFader(6), LEDFader(9), LEDFader(10),
...
void setup() {
  ...
  attachInterrupt(interruptPin, sample, RISING); // begin sampling from
                                                 // interrupt
}
...
void checkMenu() {
  ...
  if ((currentMillis - previousMillis) > menuTimeout) { // menu timeout!!
    ...
    if (maxBrightness > 1)
      noteLEDs = 1; // normal light show, or leave off
    leds[prevValue].stop_fade();
    leds[prevValue].set_value(0);
  }
}
...
void thresholdMode() {
  int runMode = 1;
  noteLEDs =
      2; // turn on special Note visualization for feedback on threshold effect
  while (runMode) {
    // float knobValue
...
void analyzeSample() {
  ...
  if (index == samplesize) { // array is full
    ...
    if (change) { // set note and control vector
      ...
      int setnote = map(averg % 127, 1, 127, noteMin,
                        noteMax); // derive note, min and max note
      ...
      setControl(controlNumber, controlMessage.value, delta % 127,
                 ramp); // set the ramp rate for the control
    }
